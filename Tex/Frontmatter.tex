...
针对上述挑战，本文的核心策略是在端到端模型内部借助文本生成来引导语音生成——通过先生成目标文本再基于文本表示生成语音的方式进行任务解耦，将复杂的语音到语音直接映射分解为两个更易建模的子任务，降低建模难度并提升生成质量。同时，语音交互场景天然要求实时性，因此本文始终追求高质量与低延迟的统一。沿着这一策略，本文的研究围绕四个递进的科学问题逐步展开。首先是质量问题：端到端系统能否达到级联系统的翻译质量？本文通过将已有的语音到文本翻译和语音合成预训练模型组合为端到端系统，在缺乏平行语音数据的条件下达到甚至超越级联系统的翻译质量，证明了端到端方法的可行性。在此基础上是速度问题：端到端的可行性已得到验证，但自回归解码逐帧生成语音序列速度过慢，如何在不牺牲翻译质量的前提下大幅提升解码效率？进一步，随着大语言模型的兴起，研究范围从语音翻译拓展到通用语音对话，产生了实时交互问题：大语言模型串行地先生成文本再合成语音导致延迟过高，如何实现文本与语音的同步流式生成以满足实时交互的需求？最后，前述工作均假设输入语音已按句切分，而真实对话场景中用户的语音是连续不断的，模型需要在持续接收音频的同时自主决策何时回复、何时停止，这引出了全双工交互问题。本文围绕上述四个问题，共完成五项具体工作。
...
To address these challenges, the core strategy of this thesis is to leverage text generation to guide speech generation within end-to-end models---decomposing the complex speech-to-speech mapping into two more tractable subtasks by first generating target text and then generating speech based on the text representation, thereby reducing modeling difficulty and improving generation quality. Meanwhile, real-time speech interaction inherently demands low latency, so this thesis consistently pursues the unification of high quality and low latency. Following this strategy, the research progresses through four successive scientific problems. The first is the quality problem: can an end-to-end system match the translation quality of cascaded systems? This thesis composes existing pretrained speech-to-text translation and text-to-speech models into an end-to-end system, achieving translation quality comparable to or exceeding cascaded systems even without parallel speech data, thereby demonstrating the feasibility of the end-to-end approach. Building upon this feasibility, the second is the speed problem: end-to-end translation is viable, but autoregressive decoding generates speech tokens frame by frame and is too slow---how can decoding efficiency be substantially improved without sacrificing translation quality? Further, with the rise of large language models, the research scope expands from speech translation to general spoken dialogue, giving rise to the real-time interaction problem: sequentially generating text first and then synthesizing speech leads to excessive latency---how can text and speech be generated synchronously in a streaming fashion to meet real-time interaction requirements? Finally, all the aforementioned works assume that input speech has been pre-segmented into utterances, whereas in real conversations, user speech arrives as a continuous stream, and the model must autonomously decide when to respond and when to stop while continuously receiving audio, which leads to the full-duplex interaction problem. This thesis addresses these four problems through five concrete works.
