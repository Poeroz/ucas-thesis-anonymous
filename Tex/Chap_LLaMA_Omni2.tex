...
接下来需要将文本对话转换为语音。与上一章使用CosyVoice和VITS合成语音不同，本章使用更先进的语音合成方案来提升合成语音的质量和多样性。为模拟真实应用场景，指令端采用多样化的声音，而响应端保持统一的声音。对于每条多轮对话，首先使用fish-speech\citep{fish-speech-v1.4}模型合成一段短语音（例如"这是一段随机生成的声音"），此时fish-speech会产生一个随机声音。随后，将该合成语音作为声纹提示，使用CosyVoice 2模型在克隆该声纹的条件下将对话中所有轮次的指令合成为语音。这种两步合成策略确保了同一对话内不同轮次的指令声音保持一致，同时不同对话之间具有声音多样性，使训练数据覆盖各种声音特征。对于所有响应，使用统一的声纹提示通过CosyVoice 2模型合成语音，保持模型输出语音风格的一致性。
...
表\ref{tab:llamaomni2_readwrite}展示了不同读写策略组合的对比结果。读写策略通过$\mathcal{R}$和$\mathcal{W}$两个超参数控制语音生成的粒度和节奏，直接影响语音质量和系统延迟之间的权衡。
