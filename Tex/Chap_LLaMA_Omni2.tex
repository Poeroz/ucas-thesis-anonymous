...
\textbf{第一阶段}\quad 第一阶段分别训练语音到文本和文本到语音两个方向的能力。训练数据由多轮语音对话数据中提取的语音指令-文本响应对和文本响应-语音响应对组成。对于语音到文本方向（阶段I(a)），冻结语音编码器，训练语音适配器和大语言模型，使用交叉熵损失。该阶段使模型学会理解语音指令并生成高质量的文本响应。对于文本到语音方向（阶段I(b)），训练TTS语言模型，使用交叉熵损失。该阶段使TTS语言模型学会根据文本输入生成对应的语音词元。在此阶段中，门控融合模块不参与训练，$\mathcal{M}_\text{TTS}$仅接收文本词元嵌入作为输入，尚未使用大语言模型的隐状态。这样设计是为了让TTS语言模型先建立起稳定的文本到语音映射能力，为后续融合隐状态信息奠定基础。

\textbf{第二阶段}\quad 第二阶段训练模型的语音到语音生成能力，使用完整的语音对话数据。此阶段冻结语音编码器、语音适配器和大语言模型的所有参数，仅训练门控融合模块和$\mathcal{M}_\text{TTS}$。由于大语言模型已在第一阶段完成训练并固定，其隐状态提供了稳定的输入信号，门控融合模块可以在此基础上学习如何有效地融合隐状态和文本嵌入。通过这种分阶段训练策略，TTS语言模型在第一阶段已获得基本的语音合成能力，在第二阶段则进一步学习利用大语言模型的隐状态信息来提升语音生成质量。
