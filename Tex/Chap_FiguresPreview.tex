...
\begin{tikzpicture}[node distance=0.35cm,
    ...
    \end{scope}
    \node[right=0.15cm of enc_layer, font=\scriptsize, blue!70] {$\times N$};
    % 编码器输出
    ...
    \end{scope}
    \node[right=0.15cm of dec_layer, font=\scriptsize, red!70] {$\times N$};
    % 输出
...
\begin{tikzpicture}[node distance=0.3cm and 0.3cm,
    ...
    % === (c) 串联双编码器 ===
    \def\xc{10.0}
    \node[enc=green] (c_senc) at (\xc, -0.15) {语音编码器};
    \node[enc=orange] (c_tenc) at (\xc, 1.0) {文本编码器};
    \node[enc=red] (c_dec) at (\xc, 2.15) {文本解码器};
    \node[font=\scriptsize] at (\xc, \inputY-0.45) {源语言语音};
