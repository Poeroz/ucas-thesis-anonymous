...
上述三种建模范式中，不论是两阶段范式还是端到端范式，都涉及语音到文本生成和语音到语音生成两个核心环节。因此，如图\ref{fig:rw_overview}所示，本章将分别从编码器-解码器和仅解码器两种架构出发，在每种架构下先介绍语音到文本生成的相关研究，再介绍语音到语音生成的相关研究。在此之前，第\ref{sec:rw_repr}节先介绍语音表征的相关研究，为后续内容提供前置知识。最后，第\ref{sec:rw_analysis}节对研究现状进行综合分析，指出当前研究中存在的关键问题，引出本文的研究内容。
...
% 图：语音到文本翻译模型结构对比图
% 设计原则：三子图整体垂直居中；所有箭头统一长度（标签↔方框 = 方框↔方框 = 0.5cm）
\begin{figure}[htbp]
\centering
\begin{tikzpicture}[node distance=0.3cm and 0.3cm,
    ...
    % 统一间距参数
    \def\gap{0.5}        % 方框之间、方框与标签之间的统一间距
    \def\titleGap{0.7}   % 底部标签与标题的间距

    % === (c) 串联双编码器 — 最高的子图，先定位以确定全局范围 ===
    % 从下到上：输入标签 → 语音编码器 → 文本编码器 → 文本解码器 → 输出标签
    % 总高度 = 4*gap + 3*boxH = 4*0.5 + 3*0.7 = 4.1（以中心算）
    % 垂直居中：中心在 文本编码器 的位置
    \def\xc{10.0}
    \pgfmathsetmacro{\cMid}{0}          % 垂直中心 y
    \pgfmathsetmacro{\cTenc}{\cMid}      % 文本编码器 y（中心）
    \pgfmathsetmacro{\cSenc}{\cMid-0.7-\gap}   % 语音编码器 y
    \pgfmathsetmacro{\cDec}{\cMid+0.7+\gap}    % 文本解码器 y
    \pgfmathsetmacro{\cIn}{\cSenc-0.35-\gap}    % 输入标签 y
    \pgfmathsetmacro{\cOut}{\cDec+0.35+\gap}    % 输出标签 y
    \pgfmathsetmacro{\titleY}{\cIn-\titleGap}   % 标题 y（三图共用）
    ...
    % === (a) 单编码器 — 垂直居中对齐 ===
    % 从下到上：输入标签 → 语音编码器 → 文本解码器 → 输出标签
    % 总高度 = 3*gap + 2*boxH = 3*0.5 + 2*0.7 = 2.9
    % 居中：中心在 语音编码器与文本解码器之间
    \def\xa{0}
    \pgfmathsetmacro{\aMid}{\cMid}      % 与(c)共享垂直中心
    \pgfmathsetmacro{\aEnc}{\aMid-0.35-\gap/2}  % 语音编码器 y
    \pgfmathsetmacro{\aDec}{\aMid+0.35+\gap/2}  % 文本解码器 y
    \pgfmathsetmacro{\aIn}{\aEnc-0.35-\gap}     % 输入标签 y
    \pgfmathsetmacro{\aOut}{\aDec+0.35+\gap}    % 输出标签 y
