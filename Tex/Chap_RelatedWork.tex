...
\begin{tikzpicture}[
    ...
    \node[box=blue, minimum width=3.2cm] (encdec) at (2.5, -1.5) {编码器-解码器架构\\[-2pt]{\scriptsize（\S\ref{sec:rw_encdec_all}）}};
    ...
    \node[box=orange, inner xsep=6pt] (s2s_llm) at (11.0, -3.8) {语音到语音\\[-2pt]{\scriptsize（\S\ref{sec:rw_deconly_s2s} 语音交互大模型）}};
...
\begin{tikzpicture}[node distance=0.3cm and 0.3cm,
    ...
    % 统一 Y 坐标
    \def\decY{2.2}       % 文本解码器 y（三图统一）
    \def\outputY{3.0}    % 输出标签 y（三图统一）
    \def\titleY{-1.4}    % 子图标题 y

    % 统一底部基线：(c) 输入在 y=-0.9，其他子图也用同一底部基线
    \def\bottomY{-0.9}   % 所有子图共享的底部基线 y

    % === (a) 单编码器 ===
    \def\xa{0}
    \def\encYa{0.8}      % 编码器 y
    \def\inputYa{-0.1}   % 输入 y
    \node[enc=green] (a_senc) at (\xa, \encYa) {语音编码器};
    \node[enc=red] (a_dec) at (\xa, \decY) {文本解码器};
    \node[font=\scriptsize] (a_in) at (\xa, \inputYa) {源语言语音};
    \node[font=\scriptsize] (a_out) at (\xa, \outputY) {目标语言文本};
    \draw[arrow2] (a_in) -- (a_senc);
    \draw[arrow2] (a_senc) -- (a_dec);
    \draw[arrow2] (a_dec) -- (a_out);
    \path (\xa, \bottomY) coordinate (a_phantom);  % 底部基线占位
    \node[font=\small\bfseries] at (\xa, \titleY) {(a) 单编码器};

    % === (b) 并联双编码器 ===
    \def\xb{5.0}
    \def\encYb{0.8}      % 编码器 y
    \def\inputYb{-0.1}   % 输入 y
    \node[enc=green] (b_senc) at (\xb-1.4, \encYb) {语音编码器};
    \node[enc=orange] (b_tenc) at (\xb+1.4, \encYb) {文本编码器};
    \node[enc=red] (b_dec) at (\xb, \decY) {文本解码器};
    \node[font=\scriptsize] (b_in1) at (\xb-1.4, \inputYb) {源语言语音};
    \node[font=\scriptsize] (b_in2) at (\xb+1.4, \inputYb) {源语言文本};
    \node[font=\scriptsize] (b_out) at (\xb, \outputY) {目标语言文本};
    ...
    \path (\xb, \bottomY) coordinate (b_phantom);  % 底部基线占位
    \node[font=\small\bfseries] at (\xb, \titleY) {(b) 并联双编码器};

    % === (c) 串联双编码器 ===
    \def\xc{10.0}
    \def\encYc{0.0}      % 底部编码器 y
    \def\inputYc{\bottomY}  % 输入 y（与底部编码器距离=0.9，同a/b）
    \node[enc=green] (c_senc) at (\xc, \encYc) {语音编码器};
    \node[enc=orange] (c_tenc) at (\xc, 1.1) {文本编码器};
    \node[enc=red] (c_dec) at (\xc, \decY) {文本解码器};
    \node[font=\scriptsize] (c_in) at (\xc, \inputYc) {源语言语音};
    \node[font=\scriptsize] (c_out) at (\xc, \outputY) {目标语言文本};
