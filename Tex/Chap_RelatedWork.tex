...
本章系统梳理了语音到语音生成领域的研究现状，沿着从语音翻译到语音对话的演进主线，分别综述了基于编码器-解码器架构的语音翻译和基于大语言模型的语音对话两个方向的代表性工作。综合以上研究现状，端到端语音到语音生成在这一演进过程中面临一系列递进的挑战，可归纳为以下四个关键科学问题，引出本文的五项研究工作。

在语音翻译方向，现有端到端模型需要稀缺的平行语音数据从头训练，无法像级联系统那样直接复用已有的预训练模型，翻译质量仍可能不及充分优化的级联系统。针对\textit{预训练模型的组合与利用}这一问题，本文第\ref{chap:comspeech}章提出基于模块化组合的端到端语音到语音翻译模型ComSpeech，通过CTC词表适配器打通语音到文本翻译模型和语音合成模型之间的接口壁垒，使任意预训练模型可自由组合为完整的端到端系统。此外，现有非自回归方法虽然大幅提升了解码速度，但仅在一阶段模型上进行了探索，翻译质量与自回归模型相比仍有明显差距。针对\textit{建模能力与解码效率的矛盾}，本文第\ref{chap:daspeech}章提出基于有向无环图的语音到语音翻译模型DASpeech，在两阶段框架下实现了高质量的非自回归解码，在保持翻译质量的同时大幅提升了解码速度。

在语音对话方向，尽管研究者们已将语音生成能力引入大语言模型，但现有方法在生成语音回复时通常需要串行地先生成文本再合成语音，响应延迟过高，难以满足实时对话的需求。针对\textit{多模态序列的同步流式生成}这一问题，本文第\ref{chap:llama-omni}章提出基于非自回归流式解码的实时语音对话大模型LLaMA-Omni，在大语言模型生成文本的同时同步产生语音响应，实现低延迟的实时对话；第\ref{chap:llama-omni2}章进一步提出基于自回归流式解码的高质量语音对话大模型LLaMA-Omni 2，在保持实时交互的同时大幅提升了语音的准确性和自然度。更进一步，上述模型均假设输入语音已按句切分，面向基于轮次的对话模式，而真实对话中用户的语音是连续不断的。针对\textit{双向语音流的统一序列建模}这一问题，本文第\ref{chap:full-duplex}章提出基于多信道交织的全双工语音对话模型LLaMA-Omni-Duplex，使模型能够在持续接收音频的同时自主决策回复时机。

\section{本章小结}

本章全面调研了语音到语音生成领域的研究现状。首先介绍了三种建模范式和语音表征的相关研究，随后沿着从语音翻译到语音对话的演进主线，分别梳理了两个方向的代表性工作和技术发展脉络。最后，通过综合分析指出了当前研究中存在的四个递进的关键科学问题，明确了本文五项研究工作的出发点。下一章将介绍本文的第一项工作——基于模块化组合的端到端语音到语音翻译模型ComSpeech。
