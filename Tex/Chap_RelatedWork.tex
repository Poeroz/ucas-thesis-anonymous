...
\begin{tikzpicture}[
    ...
    \node[box=teal, minimum width=3.2cm] (encdec) at (2.5, -1.5) {编码器-解码器架构\\[-2pt]{\scriptsize（\S\ref{sec:rw_encdec_all}）}};
    \node[box=orange, minimum width=3.2cm] (deconly) at (8.5, -1.5) {仅解码器架构\\[-2pt]{\scriptsize（\S\ref{sec:rw_deconly_all}）}};
    \node[box=teal] (s2tt) at (0.0, -3.8) {语音到文本\\[-2pt]{\scriptsize（\S\ref{sec:rw_encdec_s2tt} 语音到文本翻译）}};
    \node[box=blue] (s2st) at (3.7, -3.8) {语音到语音\\[-2pt]{\scriptsize（\S\ref{sec:rw_encdec_s2st} 语音到语音翻译）}};
    \node[box=orange] (s2t_llm) at (7.3, -3.8) {语音到文本\\[-2pt]{\scriptsize（\S\ref{sec:rw_deconly_s2t} 语音理解大模型）}};
    \node[box=red] (s2s_llm) at (11.0, -3.8) {语音到语音\\[-2pt]{\scriptsize（\S\ref{sec:rw_deconly_s2s} 语音交互大模型）}};
...
从系统层面来看，如图\ref{fig:rw_paradigm}所示，语音到语音生成存在三种主要的建模范式，分别对应不同的模块化程度和端到端程度。
...
\begin{tikzpicture}[node distance=0.35cm,
    ...
    \end{scope}
    \node[right=0.3cm of enc_layer, font=\scriptsize, blue!70] {$\times N$};
    % 编码器输出
    ...
    \end{scope}
    \node[right=0.3cm of dec_layer, font=\scriptsize, red!70] {$\times N$};
    % 输出
...
\begin{tikzpicture}[node distance=0.3cm and 0.3cm,
    ...
    % 统一 Y 坐标：文本解码器统一y=2.0，标题、输入、输出统一
    \def\decY{2.0}       % 文本解码器 y（三图统一）
    \def\titleY{-1.2}    % 子图标题
    \def\inputY{-0.4}    % 输入标签
    \def\outputY{2.8}    % 输出标签

    % === (a) 单编码器 ===
    \def\xa{0}
    \node[enc=green] (a_senc) at (\xa, 0.7) {语音编码器};
    \node[enc=red] (a_dec) at (\xa, \decY) {文本解码器};
    ...
    % === (b) 并联双编码器 ===
    \def\xb{5.0}
    \node[enc=green] (b_senc) at (\xb-1.4, 0.5) {语音编码器};
    \node[enc=orange] (b_tenc) at (\xb+1.4, 0.5) {文本编码器};
    \node[enc=red] (b_dec) at (\xb, \decY) {文本解码器};
    \node[font=\scriptsize] at (\xb-1.4, \inputY) {源语言语音};
    \node[font=\scriptsize] at (\xb+1.4, \inputY) {源语言文本};
    ...
    % === (c) 串联双编码器 ===
    \def\xc{10.0}
    \node[enc=green] (c_senc) at (\xc, 0.0) {语音编码器};
    \node[enc=orange] (c_tenc) at (\xc, 1.0) {文本编码器};
    \node[enc=red] (c_dec) at (\xc, \decY) {文本解码器};
    \node[font=\scriptsize] at (\xc, \inputY-0.2) {源语言语音};
...
基于编码器-解码器的语音到语音翻译（Speech-to-Speech Translation, S2ST）是语音到语音生成领域的核心任务之一，旨在通过端到端模型直接完成从源语言语音到目标语言语音的转换。按照解码策略和模型结构的不同，如图\ref{fig:rw_s2st}所示，现有S2ST方法可分为一阶段S2ST、两阶段S2ST和非自回归S2ST三类。
...
\begin{tikzpicture}[node distance=0.3cm and 0.25cm,
    ...
    \node[blk=orange, minimum width=2.0cm] (b_dec1) at (3.4, \yb) {文本解码器};
...
基于仅解码器模型的语音到文本生成旨在利用大语言模型的文本理解和生成能力处理语音输入。如图\ref{fig:rw_s2t_llm}所示，其典型架构由语音编码器、适配器（Adaptor）和大语言模型三部分组成：语音编码器（如Whisper、Wav2Vec 2.0等）提取连续语音特征，适配器将语音特征映射到大语言模型的文本嵌入空间中，大语言模型基于映射后的语音表示生成文本输出。由于语音编码器和大语言模型均可灵活替换（如语音编码器可选用Whisper或Wav2Vec 2.0，大语言模型可选用LLaMA或Qwen等），适配器的设计成为该类工作的核心差异所在。按照适配器类型的不同，现有工作可分为以下几类。
...
基于仅解码器模型的语音到语音生成是该领域的前沿方向。如图\ref{fig:rw_s2s_llm}所示，按照语音生成方式的不同，现有方法可分为原生式和组合式两类。
