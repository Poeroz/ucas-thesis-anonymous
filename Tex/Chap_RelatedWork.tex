...
如第\ref{chap:introduction}章所述，语音到语音生成的完整流程为：将输入语音波形转换为输入语音表示，由核心模型将输入语音表示映射为输出语音表示，再通过声码器将输出语音表示还原为语音波形。在这一流程中，语音表征贯穿了语音理解和语音生成的各个环节。原始语音波形包含数万赫兹采样率的时域信号，信息密度低且冗余度高，难以直接用于序列到序列的建模。因此，将语音波形转换为更紧凑、更结构化的中间表示是构建语音到语音生成系统的基础。在语音理解方面，语音表征的质量决定了模型对语义、情感等信息的捕获能力；在语音生成方面，语音表征的形式（连续或离散、单码本或多码本）决定了后续生成模型的建模范式和输出质量。本节将分别介绍用于理解的语音表征和用于生成的语音表征的相关研究。
...
SpeechGPT-Gen\citep{zhang2024speechgptgen}和VALL-E 2\citep{chen2024valle2}将语音生成解耦为语义词元和声学词元两个阶段，大语言模型仅负责生成语义词元，声学词元由外接解码器补全，降低了大语言模型的语音建模负担。Freeze-Omni\citep{xiong2024freeze}提出在语音对话训练过程中完全冻结大语言模型的参数，仅训练轻量的语音编码器和语音解码器，最大程度地保留了大语言模型原有的文本理解和推理能力，有效避免了灾难性遗忘问题。Minmo\citep{chen2025minmo}基于约8B参数的大语言模型搭配CosyVoice 2语音解码器，通过多阶段训练策略对齐语音编码器、大语言模型和语音解码器，构建了支持无缝语音对话的完整系统。Qwen2.5-Omni\citep{alibaba2025qwen25omni}提出了Thinker-Talker架构，其中Thinker组件负责接收多模态输入并进行文本推理，Talker组件基于Thinker的隐藏状态以流式方式生成语音词元，将文本推理和语音生成功能清晰解耦，在保证文本生成质量的同时实现了高质量的语音输出。VocalNet\citep{vocalnet2025}引入多词元预测策略，在每个解码步骤同时预测多个语音词元以加速语音生成，在解码效率上取得了显著提升。
